    ...
    return (
        <div className="bg-dark-surface border border-dark-border rounded-xl p-8 shadow-dark-xl space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    <label htmlFor="reg_city" className="block text-sm font-medium text-text-secondary mb-2">City</label>
                    <input
                        id="reg_city"
                        value={data.city || ''}
            ...
            <div className="pt-2">
                ...
                <p className="text-xs text-text-tertiary mb-4">
                    These details will appear on your public restaurant card on the Nearby page.
                </p>
            </div>

            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                    <label htmlFor="price_range" className="block text-sm font-medium text-text-secondary mb-2">Price Range</label>
                    <select
                        id="price_range"
                        value={data.price_range || '₹₹'}
                        onChange={(e) => setData({ ...data, price_range: e.target.value })}
                        className="w-full bg-dark-bg border border-dark-border text-text-primary py-3 px-4 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 transition-colors"
                    >
                ...
                <div>
                    <label htmlFor="price_for_two" className="block text-sm font-medium text-text-secondary mb-2">Cost for Two (₹)</label>
                    <input
                        id="price_for_two"
                        type="number"
                        value={data.price_for_two || ''}
                        onChange={(e) => setData({ ...data, price_for_two: e.target.value })}
                        placeholder="2500"
                        className="w-full bg-dark-bg border border-dark-border text-text-primary py-3 px-4 rounded-lg placeholder:text-text-tertiary focus:outline-none focus:ring-2 focus:ring-primary-500/50 focus:border-primary-500 transition-colors"
                    />
                </div>
                <div>
                    <label htmlFor="offers" className="block text-sm font-medium text-text-secondary mb-2">Walk-in Offer</label>
                    <input
                        id="offers"
                        value={data.offers || ''}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label htmlFor="latitude" className="block text-sm font-medium text-text-secondary mb-2">
                        Latitude <span className="text-text-tertiary font-normal">(for distance)</span>
                    </label>
                    <input
                        id="latitude"
                        type="text"
                ...
                <div>
                    <label htmlFor="longitude" className="block text-sm font-medium text-text-secondary mb-2">
                        Longitude <span className="text-text-tertiary font-normal">(for distance)</span>
                    </label>
                    <input
                        id="longitude"
                        type="text"
    ...
    async function submit() {
        ...
        try {
            ...
            const payload = {
                ...
                price_for_two: data.price_for_two ? Number(data.price_for_two) : null,
            ...
            // Redirect to create public listing after short delay
            setTimeout(() => {
                navigate(`/merchant/${res.data.tenant.id}/create-listing`)
            }, 2000)
        } catch (err: any) {
            ...
            <div className="w-full max-w-2xl">
                ...
                {success ? (
                    <div className="bg-dark-surface border border-green-500/30 rounded-xl p-8 shadow-dark-xl text-center space-y-4">
                        ...
                        <p className="text-text-secondary">Now let's create your public listing to appear on the Nearby page...</p>
                    </div>
                ) : (
                    <>
                        {step === 1 && (
